...
//
// As in LISP, a list can contain any type of data structure as an item on the list:
// thread control blocks, pending interrupts, etc.
//
...
//----------------------------------------------------------------------------------------
// The following class defines a "list element" -- which is used to keep track of
// one item on a list.
//
...
//----------------------------------------------------------------------------------------
// The following class defines a "list" -- a singly linked list of list elements,
// each of which points to a single item on the list.
//
// By using the "Sorted" functions, the list can be kept in sorted in increasing order
// by "key" in ListElement.
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// List::List
// Initialize a list, empty to start with. Elements can now be added to the list.
//----------------------------------------------------------------------------------------
...
// list -- this module allocates and de-allocates the ListElements to keep track of each
// item, but a given item may be on multiple lists, so we can't de-allocate them here.
//----------------------------------------------------------------------------------------
...
//      
// Allocate a ListElement to keep track of the item. If the list is empty, then this
// will be the only element. Otherwise, put it at the end.
//
...
//
// Allocate a ListElement to keep track of the item. If the list is empty, then this
// will be the only element. Otherwise, put it at the beginning.
//
// "item" is the thing to put on the list, it can be a pointer to anything.
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// List::Apply
// Apply a function to each item on the list, by walking through the list, one element
// at a time.
//
...
//----------------------------------------------------------------------------------------
// List::SortedInsert
// Insert an "item" into a list, so that the list elements are sorted in increasing
// order by "sortKey".
//
// Allocate a ListElement to keep track of the item. If the list is empty, then this
// will be the only element. Otherwise, walk through the list, one element at a time,
...
// Sets *keyPtr to the priority value of the removed item (this is needed by
// interrupt.cc, for instance).
//
// "keyPtr" is a pointer to the location in which to store the priority of the
// removed item.
//----------------------------------------------------------------------------------------

template <class Item>
Item List<Item>::SortedRemove(int *keyPtr)
{
	...
	// If list had one item, now has none.
